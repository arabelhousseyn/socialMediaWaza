...
class registerController extends Controller
{
    ...
    public function index2(Request $request)
    {
        ...
        if($validator->validated())
        {
            ...
            $checkEmail = User::on('mysql2')->where('email',$request->email)->first();

            if(!$checkEmail)
            {
                $checkPhone = User::on('mysql2')->where('phone',$request->phone)->first();
                if(!$checkPhone)
                {
                    $path = $this->ImageUpload($request->picture,'profiles');

                        $user = User::on('mysql2')->create([
                        'fullName' => $request->fullName,
                        'subName' => '',
                        'dob' => $request->dob,
                        'picture' => env('DISPLAY_PATH') .'profiles/'. $path,
                    ...
                    $usr = User::on('mysql2')->where('id',$user->id)->select('id','fullName',
                    'dob','picture','gender','profession','wilaya_id','phone','email','is_freelancer',
                    'is_verified','receive_ads','token')->first();
                    $token = $usr->createToken('my-app-token')->plainTextToken;
